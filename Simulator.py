...
class RISC_V_Simulator:
    ...
    def int_to_bin(self, num, width=32):
        width = width - 1
        if num < 0:
            num = (2 ** (width)) + num
    ...
    def rtype(self, ins):
        ...
        if funct3 == '000' and funct7 == '0000000':  # ADD
            ...
            elf.registers[self.bin_to_int(rd, 0)] = self.registers[self.bin_to_int(rs1, 0)] << self.bin_to_int(self.int_to_bin(self.registers[self.bin_to_int(rs2, 0)] )[2:][-5:], 0)
    ...
    def j_type(self, instruction):
        ...
        self.pc += self.bin_to_int(self.sext(imm, 32)) // 4
        self.pc += 1

    def execute(self, input_file):
        read = open(input_file, 'r')
        program = [line.strip() for line in read.readlines() if line]
        while self.pc < len(program):
            opcode = program[self.pc][-7:]

            if opcode == '0110011':  # r_type
                self.rtype(program[self.pc])
            elif opcode in ['0000011', '0010011', '1100111']:  # i_type
